...
import { bookGuideStateAtom } from "../../store/buku-panduan";
...
import { dictionaryStateAtom } from "../../store/kamus-kampus-gratis";

export const useGetBukuPanduan = (): UseQueryResult<TBookGuidesResponse> =>
  useQuery({
...
export const useGuideBook = (): TuseBukuPanduan => {
  const [get, set] = useAtom(bookGuideStateAtom);
  return {
...
export const useGetDetailBukuPanduan = (id: string): UseQueryResult<TBookGuidesDetailResponse> =>
  useQuery({
    queryKey: ['detail-panduan-book-get'],
    queryFn: async () => await detailPanduanBukuGetRequest(id),
  });

export const useGetDictionary = (): UseQueryResult<TBookGuidesResponse> =>
  useQuery({
...
export const useDictionary = (): TuseKamusPanduan => {
  const [get, set] = useAtom(dictionaryStateAtom);
  return {
...
export const useGetDetailDictionary = (id: string): UseQueryResult<TDictionariesDetailResponse> =>
  useQuery({
    queryKey: ['detail-panduan-dictionary-get'],
    queryFn: async () => await detailDictionaryGetRequest(id),
  });

export const useGetFaq = (): UseQueryResult<TFaqResponse> =>
  useQuery({
    queryKey: ['faq-get'],
