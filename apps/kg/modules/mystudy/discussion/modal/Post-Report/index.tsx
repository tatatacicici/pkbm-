...
export const PostReportModal: FC<TPostReportProps> = ({
  ...
  const reportOption = [
    { title: 'Spam', report_type: 'SPAM' },
    { title: 'Ujaran atau simbol kebencian', report_type: 'HATE_SPEECH' },
    { title: 'Kekerasan atau organisasi berbahaya', report_type: 'VIOLENCE_DANGEROUS_ORGANIZATION' },
    { title: 'Ketelanjangan atau aktivitas seksual', report_type: 'NUDITY_SEXUAL_ACT' },
    { title: 'Penjualan barang ilegal atau barang dengan izin khusus', report_type: 'ILLEGAL_GOODS_SPECIAL_PERMIT' },
    { title: 'Perundungan (bullying) atau pelecehan', report_type: 'BULLYING' },
    { title: 'Pelanggaran hak kekayaan intelektual', report_type: 'COPYRIGHT_INFRINGEMENT' },
    { title: 'Bunuh diri atau melukai diri sendiri', report_type: 'SUICIDE_SELF_HARM' },
    { title: 'Penipuan atau penggelapan', report_type: 'SCAM' },
    { title: 'Informasi palsu', report_type: 'FALSE_INFORMATION' },
    { title: 'Saya hanya tidak menyukainya', report_type: 'I_DONT_LIKE_THIS' },
  ];
    ...
    try {
      type === 'discussion'
        ...
        ? mutateComment(reportPayload, {
            onSuccess: () => {
              queryClient.invalidateQueries(['comment-get-by-id', getSeletedPostId]);
              setReportDetailValue('');
              setReportDetail('');
              setSelectedOption('reportSuccess');
            },
          ...
          <main className="text-sm py-4 flex flex-col gap-4 items-center text-center">
            ...
            <div className="w-full flex justify-end">
              <Button
                ...
                type="submit"
                className="relative z-10 flex items-center justify-center gap-2 px-4 py-2 text-sm font-bold duration-300 ease-in-out border-2 rounded-md border-version2-500 disabled:bg-neutral-300 bg-sky-base text-neutral-100 hover:opacity-75 transition-all"
              >
                Lapor
              </Button>
            </div>
