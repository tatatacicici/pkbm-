...
export const QuizTakeModule = () => {
  ...
  const data = quizTesBakatTakeDummyData;
  ...
  function createResult() {
    ...
    const totalQuestions = quizTesBakatInfoDummyData.quiz.total_questions;
    const questionPoint = quizTesBakatInfoDummyData.quiz.question_point;
    const remainingAttempt = quizTesBakatInfoDummyData.quiz.remaining_attempts;
  ...
  const handleSubmit = () => {
    if (!isAlreadySubmit) {
      ...
      router.push(
        `${pathname}/nilai-quiz?id=${result.id}&score=${result.score}&right=${result.right}&wrong=${result.wrong}&time_elapsed=${result.time_elapsed}&total_question=${result.total_question}&remaining_attempt=${result.remaining_attempt}&status=${result.status}`
      );
