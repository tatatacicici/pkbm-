...
export const QuizDetailModule = () => {
  ...
  //   quizId?.toString() || ''
  // );
  const data = { data: quizTesBakatInfoDummyData };
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const storedStatus = localStorage.getItem('selectedAnswers-tes-bakat');
  ...
  useEffect(() => {
    if (
      quizTesBakatInfoDummyData.subject.name &&
      quizTesBakatInfoDummyData.quiz.title
    ) {
      let title = quizTesBakatInfoDummyData.quiz.title.trim();
      ...
      document.title = `${title} - ${quizTesBakatInfoDummyData.quiz.categoryQuiz} | Paket-C Gratis`;
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="bg-blue-50 border-l-4 border-blue-500 p-2 sm:p-5 mb-3 sm:mb-4">
              <h3 className="text-lg sm:text-md md:text-lg font-bold text-sky-base mb-1">
                Tentang {quizDesc.categoryQuiz}
              </h3>
              <p className="text-gray-600 text-xs sm:text-sm">
                Tes bakat yang membantu Anda mengenali potensi dan kemampuan
            ...
            <div className="space-y-2 sm:space-y-3 text-gray-700 text-xs sm:text-sm">
              <p className="text-sm sm:text-base">
                Tes ini bertujuan untuk membantu Anda mengenali bidang yang
                sesuai dengan minat dan kecenderungan pribadi.
              </p>
              <p className="text-sm sm:text-base">
              ...
              <p className="text-sm sm:text-base">
                ...
                yang harus dikerjakan dalam tes ini.
              </p>
              ...
              <p className="text-sm sm:text-base font-bold">
                {/* Syarat Nilai Kelulusan : {quizDesc.passing_score} Poin */}
                {/* <br /> */}
                Durasi Tes Bakat : {formatDuration(quizDesc.duration / 60)}
              </p>
              <p className="text-sm sm:text-base">
            ...
            <div className="mt-4 sm:mt-6 space-y-2 sm:space-y-3">
              ...
              {quizDesc.remaining_attempts > 0 && !isQuizInProgress && (
                <Link
                  // href={`${pathname}/mulai?total_question=${quizDesc.total_questions}`}
                  href={`/tes-minat-bakat/bakat/session/sessionId/quiz/quizId/mulai?total_question=${quizDesc.total_questions}`}
                  className="block"
                  onClick={() => {
                    localStorage.setItem(
                      'selectedAnswers-tes-bakat',
              ...
              {quizDesc.remaining_attempts < 20 && (
                <Link
                  ...
                  href={`/tes-minat-bakat/bakat/session/sessionId/quiz/quizId/riwayat-quiz?total_question=${quizDesc.total_questions}`}
                  className="block"
                >
