...
import { useGetQuizHistoryById } from '@kampus-gratis/apps/kg/hooks/try-out/quiz/hook';
...
export const QuizHistoryModule: FC<Props> = ({ id }): ReactElement => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const storedQuizHistory = localStorage.getItem('quizTryoutScoreHistory');
      ...
      if (storedQuizHistory) {
        try {
          ...
          console.error(
            'Gagal memparsing quizTryoutScoreHistory dari localStorage:',
            error
          );
        }
      }
  ...
  return (
    <div className="w-[90%] md:w-[85%] min-h-screen mx-auto my-20">
      <h1 className="text-lg sm:text-xl md:text-2xl text-center font-bold text-gray-900">
        Riwayat Try Out {quizData?.quiz.categoryQuiz}
      </h1>
