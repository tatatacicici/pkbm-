...
import {
  TMyStudyV2Item,
  TMyStudyv2Response,
} from '@kampus-gratis/apps/kg/types/tryout';
import { SearchInput } from './search-input';
import { quizTryoutInfoDummyData } from '../constant';
...
export const TabContentTryOutSection = (data: TMyStudyv2Response) => {
  ...
  return (
    <div className="w-full mx-auto">
      ...
      {filteredData?.length === 0 && (
        <EmptyState text="Try Out tidak ditemukan" />
      )}
      <div
        className={
                  ...
                  >
                    <div className="">
                      ...
                      <div className="mt-3">
                        ...
                        <p className="text-sm text-neutral-700">
                          {isCollege ? 'Mata Pelajaran' : 'Try Out'} -{' '}
                    ...
                    <div className="w-full space-y-2 border-t pt-2">
                      ...
                      <div className="flex justify-between gap-2">
                        ...
                        <Link
                          className="text-white bg-sky-base hover:opacity-80 transition-all text-sm font-semibold flex items-center gap-1 px-4 py-2 rounded-lg mt-2"
                          href={`/try-out/session/${(() => {
                            const quizData = quizTryoutInfoDummyData.find(
                              (item) => String(item.id) === String(subject.id)
                            );
                            return quizData?.display_session_no;
                          })()}/quiz/${(() => {
                            const quizData = quizTryoutInfoDummyData.find(
                              (item) => String(item.id) === String(subject.id)
                            );
                            return quizData?.display_id || subject.id;
                          })()}`}
                        >
                          Mulai Tes
                          <FaArrowRight />
