...
import { quizTryoutInfoDummyData, quizTryoutTakeDummyData } from '../constant';
...
export const QuizTakeModule = () => {
  ...
  // Cari data soal berdasarkan quiz_id dari query params
  const quizData = quizTryoutTakeDummyData.find(
    (item) => String(item.display_id) === String(quiz_id)
  );
  const data = quizData || null;
  ...
  const [selectedAnswers, setSelectedAnswers] = useState(() => {
    // create empty array with length of questions_answers
  ...
  const handleNext = () => {
    ...
    if (
      ...
      currentQuestion < data.data.questions_answers.length - 1
    ) {
      setCurrentQuestion(currentQuestion + 1);
      localStorage.setItem(
        `lastQuestionIndex-${id}`,
  ...
  function createResult() {
    ...
    // Cari data quiz info berdasarkan display_id
    const quizInfoData = quizTryoutInfoDummyData.find(
      (item) => String(item.display_id) === String(quiz_id)
    );

    if (!data || !quizInfoData) {
      return null;
    }

    const totalQuestions = quizInfoData.quiz.total_questions;
    const questionPoint = quizInfoData.quiz.question_point;
    const remainingAttempt = quizInfoData.quiz.remaining_attempts;
    ...
    selectedAnswers.forEach((selected, index) => {
      ...
      const correctAnswer = questionData.answers.find((ans) => ans.correct)?.id;
      ...
      if (selected.status === 'answered') {
        ...
        unanswered += 1; // Hitung jawaban kosong sebagai salah
      }
    });
