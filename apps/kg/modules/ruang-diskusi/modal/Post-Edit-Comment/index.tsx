...
export const PostEditCommentModal: FC<TPostEditCommentProps> = ({
  ...
  const { data } = useGetAllComment(getSelectedPostId);
  const { data: dataReplies } = useGetAllReplies(
    getSelectedPostId,
  ...
  const selectedCommentData = discussionData?.find(
    (comment: TCommentResponse) => comment.id === getSelectedCommentId
  );

  const selectedReplyData = dataReplies?.data?.find(
    (comment: TCommentResponse) => comment.id === getSelectedReplyId
  );
  ...
  const { mutate: mutateReply, isLoading: isLoadingReply } = useUpdateReply(
    getSelectedPostId,
    getSelectedCommentId,
    getSelectedReplyId
  );
  ...
  const validationSchema = z.object({
    content: z
      ...
      .min(1, { message: 'Isi diskusi harus diisi' }),
    image: z
      .array(
        z
          .any()
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      type === 'comment'
        ? await mutate(payload as unknown as TCommentsPayload, {
            onSuccess: () => {
              queryClient.invalidateQueries(['comment-get-by-id']);
              setOptionOpen(false);
              reset(data);
              toast.success('Berhasil mengubah komentar');
            },
          })
        : await mutateReply(data as unknown as TCommentsPayload, {
            onSuccess: () => {
              queryClient.invalidateQueries(['comment-get-by-id']);
              setOptionOpen(false);
              reset(data);
