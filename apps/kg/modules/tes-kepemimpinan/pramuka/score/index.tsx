...
export const QuizScoreModule = (): ReactElement => {
  ...
  const { data: quizScoreData, isLoading } = useGetQuizScoreById('pramuka');
  ...
  // ? | untuk menyimpan nilai skor user
  useEffect(() => {
    const storedQuizScore = localStorage.getItem('quizPramukaScoreHistory');
    if (storedQuizScore) {
      try {
        ...
        console.error('Error parsing quizPramukaScoreHistory:', error);
      }
    }
  }, []);
  ...
  const scoreQuizBreadCrumb = [
    ...
    {
      name: 'Hasil Kuis',
      link: `/tes-kepemimpinan/pramuka/session/sessionId/quiz/quizId/mulai/nilai-quiz`,
    },
  ];
                    ...
                    <div className=" w-full flex justify-center gap-y-3 lg:justify-end mt-10 flex-wrap gap-x-2">
                      <div className="w-full">
                        ...
                        >
                          <button
                            type="button"
                            className="text-sky-base border-2 border-sky-base bg-white hover:opacity-80 w-full h-12 rounded-md shadow-sm font-bold transition-all ease-out duration-300"
                          >
                      ...
                      {dataQuizScores?.remaining_attempt > 0 && (
                        <div className="w-full ">
                          ...
                          >
                            <button
                              type="button"
                              className="bg-sky-base border-2 border-sky-base text-white hover:opacity-80 w-full h-12 rounded-md shadow-sm font-bold transition-all ease-out duration-300"
                            >
                              Coba Kembali (Sisa{' '}
