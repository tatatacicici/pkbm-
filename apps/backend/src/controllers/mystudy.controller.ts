...
export class MyStudyController {
    ...
    try {
      ...
      return successResponse(res, 'My study data retrieved successfully', {
        ...
        stats: {
          ...
          enrolled_subjects: userSubjects.filter((s: any) => s.enrolled_at)
            .length,
          completed_subjects: userSubjects.filter((s: any) => s.completed_at)
            .length,
        },
      });
    } catch (error: any) {
  ...
  async getQuizDetail(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return successResponse(
        res,
        'Quiz detail retrieved successfully',
        quiz[0]
      );
    } catch (error: any) {
      return errorResponse(
  ...
  async submitQuiz(req: Request, res: Response): Promise<Response> {
    try {
      ...
      for (const answer of answers) {
        ...
        const isCorrect =
          correctAnswer.length > 0 && (correctAnswer[0] as any).is_correct;
  ...
  async getQuizResult(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return errorResponse(
        res,
        'Failed to retrieve quiz result',
        500,
        error.message
      );
    }
